...
from accounting_sync.utils.fennoa_client import FennoaClient


class Command(BaseCommand):
    help = 'Sync receivables and payables from Fennoa'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Starting Fennoa sync from {start_date} to {end_date}")
        ...
        self.stdout.write(self.style.SUCCESS('Fennoa sync completed'))

    def _sync_receivables(self, client: FennoaClient, start_date: str, end_date: str):
        """Sync sales invoices (receivables) from Fennoa."""
        sync_log = SyncLog.objects.create(
            source_system=ERPSystem.FENNOA,
        ...
        try:
            self.stdout.write("Fetching sales invoices from Fennoa...")
            ...
            while True:
                invoices = client.get_sales_invoices(start_date, end_date, page, per_page)
                if not invoices:
                    break
            ...
            for invoice_data in all_invoices:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Failed to process invoice {invoice_data.get('invoice_number', invoice_data.get('id'))}: {str(e)}"
                        )
                    )
    ...
    def _process_receivable(self, invoice_data: dict, client: FennoaClient) -> bool:
        """Process a single receivable (sales invoice). Returns True if created, False if updated."""
        external_id = str(invoice_data.get('id', ''))
        if not external_id:
            return False

        invoice_number = invoice_data.get('invoice_number', invoice_data.get('number', ''))

        invoice_date_str = invoice_data.get('invoice_date', invoice_data.get('date', ''))
        due_date_str = invoice_data.get('due_date', invoice_data.get('dueDate', ''))
        ...
        amount_gross = Decimal(str(invoice_data.get('total', invoice_data.get('amount', 0))))
        amount_tax = Decimal(str(invoice_data.get('tax', invoice_data.get('vat', 0))))
        amount_net = amount_gross - amount_tax

        amount_paid = Decimal(str(invoice_data.get('paid', invoice_data.get('paid_amount', 0))))
        ...
        self._process_receivable_lines(receivable, invoice_data.get('lines', []))
    ...
    def _process_receivable_lines(self, receivable: Receivable, lines_data: list):
        ...
        for idx, line_data in enumerate(lines_data):
            ...
            unit_price = Decimal(str(line_data.get('unit_price', line_data.get('price', 0))))
            tax_percent = Decimal(str(line_data.get('tax_percent', line_data.get('vat_percent', 0))))

            amount_net = Decimal(str(line_data.get('amount', quantity * unit_price)))
    ...
    def _sync_payables(self, client: FennoaClient, start_date: str, end_date: str):
        """Sync purchase invoices (payables) from Fennoa."""
        sync_log = SyncLog.objects.create(
            source_system=ERPSystem.FENNOA,
            sync_type='payables',
            started_at=timezone.now(),
        ...
        try:
            self.stdout.write("Fetching purchase invoices from Fennoa...")
            ...
            while True:
                invoices = client.get_purchase_invoices(start_date, end_date, page, per_page)
                if not invoices:
                    break
    ...
    def _process_payable(self, invoice_data: dict, client: FennoaClient) -> bool:
        ...
        vendor = None
        vendor_data = invoice_data.get('vendor', invoice_data.get('supplier', {}))
        if vendor_data:
        ...
        self._process_payable_lines(payable, invoice_data.get('lines', []))
